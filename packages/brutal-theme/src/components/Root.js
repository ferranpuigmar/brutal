...
import ProjectsFake from "./pages/ProjectsFake";
...
const Root = ({ state, actions }) => {
  ...
  const objPageIDs = Object.values(state.source.page).find(
    (page) => page.link === data.link
  );
  ...
  const handleFooterHeight = () => {
    if (footerRef.current) {
      setTimeout(
        () => setMainMarginBottom(footerRef.current.offsetHeight),
        400
      );
    }
  };
  ...
  const checkRedirect = (link) => {
    if (link === "/proyectos/") {
      window.location.assign("/listado-proyectos/");
      return;
    }
  };

  useEffect(() => {
    checkRedirect(data.link);
    ...
    window.onresize = () =>
      screen.width <= breakpoints.md
        ? setMovilWidth(true)
        : setMovilWidth(false);
    !footerFields &&
      setFooterFields(
        state.source.get(`/globaloptions/${state.theme.globalOptions}/`).acf
          .footer_fields
      );
  }, []);
        ...
        <GridThemeProvider gridTheme={gridTheme}>
          ...
          <Main style={{ marginBottom: mainMarginBottom }}>
            <Switch>
              ...
              <Projects
                when={data.isPostType && data.link === "/listado-proyectos/"}
              />
              <ProjectsFake when={data.link === "/proyectos/"} />
              <Error404 when={data.is404} />
            </Switch>
          </Main>

          <div aria-hidden="false" ref={footerRef} className={cx(footerFixed)}>
            <Footer
              footerFields={footerFields ? footerFields : {}}
