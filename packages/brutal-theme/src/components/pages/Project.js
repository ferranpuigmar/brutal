...
const Project = ({ state, actions, libraries, params }) => {
  ...
  const renderServices = (post) => {
    const allServices = state.source.get(
      `/categories/${state.theme.services}/`
    ).items;
  ...
  return data.isReady ? (
    <PageWrapper className={cx(pageWrapperStyle)}>
      <Container className="projectpage">
        <Title className={cx(projectTitle)} level={1}>
          {post.title.rendered}
        </Title>
        <Row>
      ...
      {portfolio && (
        <PortfolioList
          projectName={post.title.rendered}
          portfolio={portfolio}
        />
      )}
...
const PortfolioList = ({ portfolio, projectName }) => {
  ...
  return (
    <div>
      {portfolio.map((portfolioImage, index) => (
        <PortFolioImageWrapper key={uuid_v4()}>
          <CustomImage
            srcSet={portfolioImage?.imagen.sizes}
            src={portfolioImage?.imagen.url}
            alt={portfolioImage?.imagen.title ?? `Portfolio ${projectName}`}
          />
